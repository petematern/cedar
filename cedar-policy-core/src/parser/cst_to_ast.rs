...
impl Node<Option<cst::Policies>> {
    ...
    pub fn to_policyset(&self) -> Result<ast::PolicySet> {
        self.to_policyset_with_registry(None)
    }

    /// convert `cst::Policies` to `ast::PolicySet` with support for custom decision types
    pub fn to_policyset_with_registry(
        &self,
        registry: Option<&DecisionTypeRegistry>,
    ) -> Result<ast::PolicySet> {
        ...
        for (policy_id, policy) in self.with_generated_policyids()? {
            ...
            match policy.to_policy_or_template_with_registry(policy_id, registry) {
...
impl Node<Option<cst::Policy>> {
    ...
    pub fn to_policy_or_template(
        ...
        self.to_policy_or_template_with_registry(id, None)
    }

    /// Convert `cst::Policy` to an AST `StaticPolicy` or `Template` with custom decision type support
    pub fn to_policy_or_template_with_registry(
        &self,
        id: ast::PolicyID,
        registry: Option<&DecisionTypeRegistry>,
    ) -> Result<Either<ast::StaticPolicy, ast::Template>> {
        let t = self.to_policy_template_with_registry(id, registry)?;
        if t.slots().count() == 0 {
    ...
    pub fn to_policy_template(&self, id: ast::PolicyID) -> Result<ast::Template> {
        self.to_policy_template_with_registry(id, None)
    }

    /// Convert `cst::Policy` to `ast::Template` with custom decision type support
    pub fn to_policy_template_with_registry(
        &self,
        id: ast::PolicyID,
        registry: Option<&DecisionTypeRegistry>,
    ) -> Result<ast::Template> {
        ...
        // convert effect (with registry support for custom decisions)
        let maybe_effect = policy.effect.to_effect_with_registry(registry);
...
impl Node<Option<cst::Ident>> {
    ...
    pub(crate) fn to_effect(&self) -> Result<ast::Effect> {
        self.to_effect_with_registry(None)
    }

    /// Convert CST identifier to Effect, with support for custom decision types
    pub(crate) fn to_effect_with_registry(
        &self,
        registry: Option<&DecisionTypeRegistry>,
    ) -> Result<ast::Effect> {
        ...
        match effect {
            ...
            cst::Ident::Ident(name) => {
                // Check if this is a custom decision type
                if let Some(reg) = registry {
                    if let Some(id) = reg.get_id(name) {
                        return Ok(ast::Effect::Custom(id));
                    }
                }
                // Not a recognized custom decision type
                Err(self
